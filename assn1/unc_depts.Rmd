...
cs_hires <- summarize(
  group_by(
    ...
    hiredate), 
  count = n())
for (year in min(cs_hires$hiredate):max(cs_hires$hiredate)) {
  if (!(year %in% cs_hires$hiredate)) {
    add_row(cs_hires, hiredate = year, count = 0)
  }
}
...
more_hires <- summarize(
  group_by(
    mutate(
      select(
        filter(data, dept == "Computer Science" | dept == "SILS" | dept == "Math" | dept == "STOR" | dept == "Biostatistics" | dept == "Physics"),
        hiredate, dept), 
      hiredate = as.integer(hiredate / 10000)), 
    hiredate), 
  count = n())
